...
class TtmlTextParser:
    ...
    def parseMedia(self, data: bytes, time: TimeContext) -> List[Cue]:
        ...
        if xml is None:
            return cues
        parsererrors = xml.getElementsByTagName('parsererror') # type: List[Element]
        ...
        regionElements = tt.getElementsByTagName('region') # type: List[Element]
        cueRegions = []
        ...
        for divNode in body.childNodes:
            ...
            for pChildren in divNode.childNodes:
                ...
                if pChildren.tagName == 'p':
                    has_p = True
                    cue = TtmlTextParser.parseCue_(pChildren, time.periodStart, rateInfo, metadataElements, styles, regionElements, cueRegions, whitespaceTrim, False, cellResolutionInfo)
                    if cue:
                        cues.append(cue)
            if not has_p:
                cue = TtmlTextParser.parseCue_(divNode, time.periodStart, rateInfo, metadataElements, styles, regionElements, cueRegions, whitespaceTrim, False, cellResolutionInfo)
                if cue:
                    cues.append(cue)
        return cues
    ...
    def addStyle_(cue, cueElement, region, imageElement: Element, styles: List[Element], isNested: bool, isLeaf: bool):
        ...
        _writingMode = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'writingMode', shouldInheritRegionStyles)
        ...
        align = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'textAlign', shouldInheritRegionStyles)
        if align:
            cue.positionAlign = textAlignToPositionAlign_[align]
            cue.lineAlign = textAlignToLineAlign_[align]

            assert textAlign.__members__.get(align.upper()), f'{align.upper()} Should be in Cue.textAlign values!'
        else:
        ...
        backgroundColor = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'backgroundColor', shouldInheritRegionStyles)
        if backgroundColor:
            cue.backgroundColor = backgroundColor

        border = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'border', shouldInheritRegionStyles)
        if border:
            cue.border = border

        fontFamily = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'fontFamily', shouldInheritRegionStyles)
        if fontFamily:
        ...
        lineHeight = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'lineHeight', shouldInheritRegionStyles)
        if lineHeight and unitValues_.match(lineHeight):
            cue.lineHeight = lineHeight

        fontSize = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'fontSize', shouldInheritRegionStyles)
        ...
        letterSpacing = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'letterSpacing', shouldInheritRegionStyles)
        if letterSpacing and unitValues_.match(letterSpacing):
            cue.letterSpacing = letterSpacing

        linePadding = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'linePadding', shouldInheritRegionStyles)
        if linePadding and unitValues_.match(linePadding):
            cue.linePadding = linePadding

        opacity = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'opacity', shouldInheritRegionStyles)
        if opacity:
            cue.opacity = float(opacity)

        textDecorationRegion = TtmlTextParser.getStyleAttributeFromRegion_(region, styles, 'textDecoration')
        if textDecorationRegion:
            TtmlTextParser.addTextDecoration_(cue, textDecorationRegion)

        textDecorationElement = TtmlTextParser.getStyleAttributeFromElement_(cueElement, styles, 'textDecoration')
    ...
    def getStyleAttribute_(cueElement, region, styles, attribute, shouldInheritRegionStyles=True):
        attr = TtmlTextParser.getStyleAttributeFromElement_(cueElement, styles, attribute)
        if attr:
            return attr
    ...
    def getInheritedStyleAttribute_(element: Element, styles, attribute):
        ...
        for inheritedStyle in inheritedStyles:
            ...
            styleAttributeValue = inheritedStyle.getAttributeNS(ebuttsNs, attribute)

            if not styleAttributeValue:
                # Fall back to tts namespace.
                styleAttributeValue = inheritedStyle.getAttributeNS(ttsNs, attribute)

            if not styleAttributeValue:
                # Next, check inheritance.
                # Styles can inherit from other styles, so traverse up that chain.
                styleAttributeValue = TtmlTextParser.getStyleAttributeFromElement_(inheritedStyle, styles, attribute)
    ...
    def getElementsFromCollection_(element: Element, attributeName: str, collection: list, prefixName: str, nsName: str = None):
        ...
        if not element or len(collection) < 1:
            return items
        attributeValue = TtmlTextParser.getInheritedAttribute_(element, attributeName, nsName)
...
# 50.17% 10%
percentValues_ = re.compile('^(\d{1,2}(?:\.\d+)?|100(?:\.0+)?)% (\d{1,2}(?:\.\d+)?|100(?:\.0+)?)%$')
...
timeColonFormatFrames_ = re.compile('^(\d{2,}):(\d{2}):(\d{2}):(\d{2})\.?(\d+)?$')

# 00:00:40 or 00:40
timeColonFormat_ = re.compile('^(?:(\d{2,}):)?(\d{2}):(\d{2})$')

# 01:02:43.0345555 or 02:43.03
timeColonFormatMilliseconds_ = re.compile('^(?:(\d{2,}):)?(\d{2}):(\d{2}\.\d{2,})$')
...
# 3.45h, 3m or 4.20s
timeHMSFormat_ = re.compile('^(?:(\d*(?:\.\d*)?)h)?(?:(\d*(?:\.\d*)?)m)?(?:(\d*(?:\.\d*)?)s)?(?:(\d*(?:\.\d*)?)ms)?$')
